...
import { api, setStoredLanguage, getApiBaseUrl } from '../api/client';
...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({
  ...
  // Server & AI settings
  const [serverUrl, setServerUrl] = useState(localStorage.getItem('weathergpt_server_url') || '');
  const [groqKey, setGroqKey] = useState(localStorage.getItem('weathergpt_groq_key') || '');
  const [geminiKey, setGeminiKey] = useState(localStorage.getItem('weathergpt_gemini_key') || '');
  const [isTestingServer, setIsTestingServer] = useState(false);
  const [serverTestResult, setServerTestResult] = useState<{ status: 'idle' | 'success' | 'error'; message: string }>({ status: 'idle', message: '' });
  ...
  const handleSaveServerUrl = async () => {
    ...
    if (trimmed) {
      localStorage.setItem('weathergpt_server_url', trimmed);
    } else {
      localStorage.removeItem('weathergpt_server_url');
    }
    triggerSuccess();
    testServerConnection(trimmed);
  };

  const handleSaveAiKeys = () => {
    if (groqKey.trim()) {
      localStorage.setItem('weathergpt_groq_key', groqKey.trim());
    } else {
      localStorage.removeItem('weathergpt_groq_key');
    }

    if (geminiKey.trim()) {
      localStorage.setItem('weathergpt_gemini_key', geminiKey.trim());
    } else {
      localStorage.removeItem('weathergpt_gemini_key');
    }
    triggerSuccess();
  };
  ...
  const testServerConnection = async (customUrl?: string) => {
    ...
    const target = customUrl !== undefined ? customUrl : serverUrl;
    const cleanTarget = target.trim().replace(/\/$/, '');
    const urlToTest = cleanTarget ? (cleanTarget.endsWith('/api') ? `${cleanTarget}/health` : `${cleanTarget}/api/health`) : `${getApiBaseUrl()}/health`;
    ...
    try {
      ...
      if (res.ok) {
        setServerTestResult({ status: 'success', message: 'Backend connected successfully!' });
      } else {
        setServerTestResult({ status: 'error', message: `Server returned status ${res.status}` });
      }
    } catch {
      setServerTestResult({ status: 'error', message: 'Could not reach backend. Verify URL & port.' });
    } finally {
