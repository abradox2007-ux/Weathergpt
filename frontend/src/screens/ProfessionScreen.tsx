...
export const ProfessionScreen: React.FC<ProfessionScreenProps> = ({
  ...
  const getSeverityBadge = (sev: string) => {
    switch (sev) {
      case 'critical':
        return <span className="text-[10px] uppercase font-extrabold px-2.5 py-0.5 rounded-full bg-rose-100 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800">Action Required</span>;
      case 'attention':
        return <span className="text-[10px] uppercase font-extrabold px-2.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800">Advisory Caution</span>;
      default:
        return <span className="text-[10px] uppercase font-extrabold px-2.5 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">Optimal Window</span>;
    }
  };
    ...
    <div className="p-4 pt-3 w-full transition-colors duration-200">
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div className="text-left">
              ...
              <div className="text-sm font-black text-slate-900 dark:text-white">{currentProfObj.name}</div>
            </div>
          </div>
              ...
              <div className="space-y-1">
                {PROFESSIONS.map((p) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] opacity-75 truncate max-w-[150px]">{p.desc}</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      {advisory?.summary && (
        <div className="p-4 rounded-3xl bg-gradient-to-r from-sky-500/10 via-cyan-500/10 to-indigo-500/10 dark:from-sky-950/40 dark:to-slate-900 border border-sky-200 dark:border-sky-800/80 mb-3.5 flex items-start space-x-3 shadow-xs">
          ...
          <p className="text-xs text-sky-950 dark:text-sky-200 leading-relaxed font-semibold">{advisory.summary}</p>
        </div>
      )}
        ...
        {loading ? (
          ...
          advisory.topics.map((topic, idx) => (
            ...
            >
              ...
              <h3 className="text-sm font-black text-slate-900 dark:text-white mb-1.5">{topic.title}</h3>
              <p className="text-xs text-slate-600 dark:text-slate-300 mb-3.5 leading-relaxed font-medium">{topic.summary}</p>
              ...
              <div className="p-3.5 rounded-2xl bg-emerald-500/10 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-800/80 text-xs text-emerald-950 dark:text-emerald-200 flex items-start space-x-2.5">
                ...
                <div className="leading-snug">
                  ...
                  <span className="font-semibold">{topic.recommendation}</span>
                </div>
              </div>
            </div>
          ))
